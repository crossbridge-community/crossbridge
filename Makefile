...
abclibs_compile:
	...
	cd $(BUILD)/abclibsposix && python2 $(SRCROOT)/posix/gensyscalls.py $(SRCROOT)/posix/syscalls.changed
...
llvmtests:
	...
	python2 $(SRCROOT)/tools/llvmtestcheck.py --srcdir $(SRCROOT)/llvm-2.9/projects/test-suite/ --builddir $(BUILD)/llvm-tests/projects/test-suite/ --fpcmp $(FPCMP)> $(BUILD)/llvm-tests/passfail.txt
...
llvmtests-speccpu2006: # works only on mac!
	...
	python2 $(SRCROOT)/tools/llvmtestcheck.py --fpcmp $(FPCMP) --srcdir $(SRCROOT)/llvm-2.9/projects/test-suite/ --builddir $(BUILD)/llvm-tests/projects/test-suite/ > $(BUILD)/llvm-tests/passfail.txt
...
ifneq (,$(findstring cygwin,$(PLATFORM)))
	...
	cd $(BUILD)/posix && python2 $(SRCROOT)/posix/gensyscalls.py $(SRCROOT)/posix/syscalls.changed
	...
	cd $(SRCROOT)/avmplus/shell && python2 ./shell_toplevel.py -config CONFIG::VMCFG_ALCHEMY_POSIX=true
...
libobjc:
	...
	python2 $(SRCROOT)/tools/build-objc.py $(SRCROOT)/cached_build/libobjc/compile.log $(SRCROOT)/cached_build/libobjc/install.log $(SRCROOT) > $(BUILD)/libobjc/build.sh
...
checkasm:
	...
	python2 $(SRCROOT)/tools/search_headers.py $(SDK) $(BUILD)/header-search
